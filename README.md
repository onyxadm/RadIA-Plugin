...
*   **Suporte a Múltiplas IAs & Conexão Híbrida:** Modelo híbrido flexível de conexão. Permite usar chaves de API próprias (BYOK) para **Google Gemini**, **OpenAI ChatGPT**, **Azure OpenAI**, **Anthropic Claude**, **GitHub Copilot**, **DeepSeek**, **Groq**, **Alibaba Qwen**, **Mistral AI**, **OpenRouter**, **LM Studio** e **Ollama** local, OU conectar-se diretamente às suas contas pessoais/corporativas de consumo (**ChatGPT Plus/Pro** e **Gemini Advanced**) via login oficial no WebView2, contornando bloqueios de rede com injeção inteligente de DOM/CSS e ponte JS-Delphi.
